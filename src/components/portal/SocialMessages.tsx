...
export function SocialMessages() {
  ...
  useEffect(() => {
    if (selectedPartner) {
      ...
      subscribeToMessages(selectedPartner.id);
    }
  }, [selectedPartner]);
  ...
  const fetchPartners = async () => {
    ...
    try {
      ...
      // Get last message for each partner (simplified for now)
      setPartners(profiles.map(p => ({ ...p, last_message: "Inicie uma conversa", last_message_time: "" })));
    } catch (error) {
  ...
  return (
    <div className="flex h-[calc(100vh-220px)] bg-white dark:bg-slate-900 rounded-2xl shadow-sm border overflow-hidden">
      {/* Sidebar: Partners */}
      <div className="w-full md:w-80 border-r flex flex-col bg-slate-50/50 dark:bg-slate-800/20">
              ...
              {partners.map((partner) => (
                ...
                >
                  ...
                  {partner.unread_count && (
                    <div className="h-5 w-5 bg-primary text-primary-foreground text-[10px] flex items-center justify-center rounded-full font-bold">
                      {partner.unread_count}
                    </div>
                  )}
                </button>
          ...
          <>
            ...
            <div className="p-4 border-b bg-white dark:bg-slate-900 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold">{selectedPartner.full_name}</h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white dark:bg-slate-800 p-2 rounded-xl rounded-tl-none border border-slate-100 dark:border-slate-700 shadow-sm flex gap-1">
