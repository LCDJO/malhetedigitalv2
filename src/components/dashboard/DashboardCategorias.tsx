      ...
      {categoriasReceita.length === 0 ? (
        <Card>
          <CardContent className="p-0">
            <EmptyState message="Nenhuma categoria para os filtros selecionados" submessage="Selecione 'Todos os tipos' para ver todas as categorias." />
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                  ...
                  {categoriasReceita.map((cat) => (
                    <div key={cat.categoria} className="space-y-1">
                      ...
                      <div className="text-[10px] text-muted-foreground pl-5 space-y-0.5">
                        <p>Arrecadado: <strong className="text-foreground">{formatCurrency(cat.arrecadado)}</strong></p>
                        <p>Em aberto: <strong className="text-destructive">{formatCurrency(cat.emAberto)}</strong></p>
                      </div>
                    </div>
                  ))}
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-sans font-semibold">Arrecadado vs Em Aberto</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={barData} layout="vertical" margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                    <XAxis type="number" tick={{ fontSize: 10, fill: "hsl(220, 10%, 48%)" }} axisLine={false} tickLine={false} tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} />
                    <YAxis type="category" dataKey="categoria" tick={{ fontSize: 11, fill: "hsl(220, 10%, 48%)" }} axisLine={false} tickLine={false} width={100} />
                    <Tooltip content={<ChartTooltip />} />
              ...
              <div className="flex items-center justify-center gap-5 mt-3">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <div className="h-2.5 w-2.5 rounded-sm" style={{ backgroundColor: "hsl(152, 55%, 38%)" }} />Arrecadado
                </div>
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <div className="h-2.5 w-2.5 rounded-sm" style={{ backgroundColor: "hsl(0, 65%, 48%)" }} />Em Aberto
                </div>
              </div>
            </CardContent>
          </Card>
